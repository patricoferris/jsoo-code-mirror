...
module Tooltip_view = struct
  ...
  let create ~dom ?offset ?get_coords ?overlap ?mount ?update ?positioned () =
    let get_coords =
      Option.map
        (fun get_coords ->
          Jv.wrap_callback (fun pos -> get_coords (Jv.to_int pos) |> coords_to_jv))
        get_coords
    in
    let o = Jv.obj [||] in
    ...
    @@ Option.map
         (fun mount -> Jv.wrap_callback (fun view -> mount (Editor.View.of_jv view)))
         mount;
    Jv.set_if_some o "update"
    @@ Option.map
         (fun update ->
          Jv.wrap_callback (fun view_up -> update (Editor.View.Update.of_jv view_up)))
         update;
    Jv.set_if_some o "positioned" @@ Option.map Jv.wrap_callback positioned;
    o
end
...
let hover_tooltip ?config source =
  ...
  let source =
    Jv.wrap_callback @@ fun view pos side ->
    let fut =
